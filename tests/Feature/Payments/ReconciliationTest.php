...
it('AC-PAY-23 names the settled fee for the rail that incurred it', function () {
    // A bank transfer with a fee: $500 of rent plus 0.75% of it, the WP-47
    // shape — the rate Authorize.Net charges for eCheck.Net.
    $payment = Payment::factory()->create([
        'lease_id' => $this->lease->id,
        'tenant_id' => $this->tenant->id,
        'amount' => '503.75',
        'convenience_fee' => '3.75',
        'method' => 'echeck',
        'gateway' => 'authorize_net',
    ...
    $this->ledger->postPayment(
        $this->lease, 'tenant', Money::fromString('503.75'),
        'Payment submitted online', $payment->id, 'pending',
    );

    $this->batches = ['B1' => [settledTransaction('60777666555', ['settleAmount' => '503.75'])]];

    $this->reconciliation->run();

    $fee = LedgerEntry::where('category', 'convenience_fee')->sole();

    expect($fee->amount->toDecimalString())->toBe('3.75')
